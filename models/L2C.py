...
class L2Classifier:
    ...
    def fit(self, X_train: np.ndarray | pd.DataFrame | spmatrix, y_train: np.ndarray | pd.DataFrame) -> 'L2Classifier':
        ...
        # Convert X_train to an appropriate numeric representation and check for invalid values.
        if isinstance(X_train, pd.DataFrame):
            X = X_train.to_numpy(dtype=np.float32)
            if not np.all(np.isfinite(X)):
                raise ValueError("There's a NaN or infinity value in X data, please clean your data first.")

        elif issparse(X_train):
            # Check sparse data values
            if not np.all(np.isfinite(X_train.data)):
                raise ValueError("There's a NaN or infinity value in X data, please clean your data first.")
            # convert to dense for the closed-form solver (cast to float32)
        ...
        # Check y_train for missing values but do NOT call np.isfinite on possibly-string labels
        y_arr = np.asarray(y_train)
        if pd.isnull(y_arr).any():
            raise ValueError("Target `y` contains missing values; please clean your data first.")
    ...
    def predict(self, X_test: np.ndarray | pd.DataFrame | spmatrix) -> np.ndarray:
        ...
        if self.classes is not None and len(self.classes) == self.n_classes:
            # Map predicted indices back to original class labels and preserve label dtype
